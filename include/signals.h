...
inline TOut unsafe_horrible_cast(TIn mIn) noexcept {
    static_assert(sizeof(TIn) >= sizeof(TOut), "Cannot use unsafe_horrible_cast<>");
    horrible_union<TOut, TIn> u;
    u.in = mIn;
    return u.out;
}
...
key_type get_hash(R (*const FunctionPtr)(Args...)) {
    return key_type(FunctionPtr, nullptr);
}
...
class connection_t {
  public:
    ...
    connection_t(std::function<void(void)>&& deleter, detail::key_type key)
    ...
    detail::key_type get_key() const { return key_; }

  private:
    std::function<void(void)> deleter_;
    detail::key_type key_;
};
...
class signal_handle_t {
  ...
  private:
    using scoped_connection_set_t = std::set<signals::scoped_connection_t>;
    bool inner_connect(void* sig, signals::scoped_connection_t&& conn) {
        bool ret = true;
...
class signal_t {
  ...
  public:
    ...
    void clear() {
        ...
        keys_.reset(new std::vector<detail::key_type>());
    }
    ...
    inline void disconnect(R (T::*const mem_fun)(Args...), T* const obj) {
        auto key = detail::get_hash(mem_fun, obj);
    ...
    inline connection_t connect(R (*const FunctionPtr)(Args...)) {
        auto key = detail::get_hash(FunctionPtr);
        auto slot = detail::make_slot(FunctionPtr);
    ...
    connection_t connect_custom(detail::key_type key, slot_t&& slot) {
        if (!slot_list_->emplace(key, std::move(slot)).second) {
            assert(!"重复注册");
            return connection_t(std::function<void(void)>(), detail::key_type());
        }
  ...
  private:
    ...
    using slot_list_t = std::map<detail::key_type, slot_t>;
    std::shared_ptr<slot_list_t> slot_list_ = std::make_shared<slot_list_t>();
    std::shared_ptr<std::vector<detail::key_type>> keys_ =
        std::make_shared<std::vector<detail::key_type>>();

    template<typename R, typename T>
    inline connection_t connect_impl(R (T::*const mem_fun)(Args...), T* const obj, std::true_type) {
        auto key = detail::get_hash(mem_fun, obj);
        auto slot = detail::make_slot(mem_fun, obj);
...
#define SIGNAL_DEFINE_ALL(sig, ...)                   \
  public:                                             \
    using sig##_t = signals::signal_t<##__VA_ARGS__>; \
